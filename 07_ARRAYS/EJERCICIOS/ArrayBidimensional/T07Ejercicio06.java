...
public class T07Ejercicio06 {
    ...
    for(fila = 0; fila < num.length; fila++) {
        for(columna = 0; columna < num[fila].length; columna++) {
            do {
                num[fila][columna] = (int)(Math.random() * 61);
    
                // Comprueba si el número generado ya está en el array.
                repetido = false;
                for (i = 0; i < num[fila].length * fila + columna; i++) { // En este caso es 10, por ejemplo si fila es 1 y columna es 3: 10 * 1 + 3 = 13 cuadraditos a recorrer.
